...
NUM_WORKERS = 1  # Can determine 'optimal' parameter from 'determine_optimal_num_workers.py'
...
TORCH_MODEL_FILENAME_EXT = ".pt" # Desired file extension for saving PyTorch models after training
TORCH_MODEL_FILENAME_LOAD = "drone_obstacle_cnn_acc_0.8571.pt" # Desired filename of PyTorch model to load
INFERENCE_MODEL_FILENAME = "drone_obstacle_cnn_acc_0.8571.pt" # Desired filename of PyTorch model to load for inference
CHECKPOINT_DIR = "checkpoints" # Directory to store the model checkpoints during training
TEST_AND_SAVE_MODEL_PT = True # Set to True if you want to test the model after training and save the model.pt file
TEST_BEST_MODEL_CKPT = True # Set to True if you want to test the best model checkpoint after training
...
SPLIT_RATIO = (0.8, 0.1, 0.1) # Ratio of train, validation, and test datasets (Default: 80%, 10%, 10%)
